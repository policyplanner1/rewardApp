  ...
  async sendToInventory(req, res) {
    ...
    try {
      ...
      const [stockRows] = await db.query(
        `SELECT product_id, vendor_id, variant_id, total_quantity, passed_quantity, failed_quantity, location, expiry_date
      ...
      const quantityToAdd = stock.passed_quantity;

      await db.query(
        `INSERT INTO inventory (product_id, vendor_id, variant_id, warehouse_id, quantity, location, expiry_date)
   VALUES (?, ?, ?, ?, ?, ?, ?)
   ON DUPLICATE KEY UPDATE quantity = quantity + VALUES(quantity), location = VALUES(location), expiry_date = VALUES(expiry_date)`,
        [
          stock.product_id,
          stock.vendor_id,
          stock.variant_id,
          stock.warehousemanager_id, 
          quantityToAdd,
          stock.location,
          stock.expiry_date,
        ]
