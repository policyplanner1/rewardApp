...
import { FiTag, FiChevronsRight } from 'react-icons/fi';

// --- SVG Icon placeholders (kept as provided) ---
  ...
  return (
    <Link
      ...
      className={`
        ...
        ${active ? 'text-white font-bold bg-opacity-90' : 'text-gray-600 hover:bg-gray-100'}
      `}
...
// Types for nav items (loose types to avoid excessive TS configuration here)
...
export default function ManagerNavbar() {
  ...
  // derive initial active flags from pathname
  const isCategoryPathActive = pathname.startsWith('/src/manager/category_management');
  const isDocumentPathActive = pathname.startsWith('/src/manager/document_management');

  // Accordion behavior: only one open at a time
  const [isCategoryManagementOpen, setIsCategoryManagementOpen] = useState<boolean>(isCategoryPathActive);
  const [isDocumentManagementOpen, setIsDocumentManagementOpen] = useState<boolean>(isDocumentPathActive);
  ...
  // Update dropdowns on pathname change — keep accordion behavior: open correct one, close the other
  useEffect(() => {
    if (isCategoryPathActive) {
      setIsCategoryManagementOpen(true);
      setIsDocumentManagementOpen(false);
    } else if (isDocumentPathActive) {
      setIsDocumentManagementOpen(true);
      setIsCategoryManagementOpen(false);
    }
    // If neither path matches, do not change the user's manual dropdown state.
    // We intentionally don't force-close both here so manual toggles persist while navigating non-related pages.
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      Icon: FiTag,
      isDropdown: true,
      initialOpen: isDocumentPathActive,
      children: [
        { href: '/src/manager/document_management/add_document', label: 'Add Document' },
        { href: '/src/manager/document_management/category_link_document', label: 'Category Link Doc' },
      ],
    },
  ];
    ...
    <nav className="fixed top-0 left-0 z-10 flex flex-col h-full transition-all duration-300 bg-white border-r border-gray-100 w-60 shadow-xl/5">
      ...
      <div className="flex-1 p-4 space-y-1 overflow-y-auto">
        {navItems.map((item, index) => {
          if (item.isDropdown) {
            // Narrow type safely
            const dd = item as NavItemDropdown;

            // determine which dropdown this is and read correct open/active values
            const isCategory = dd.label === 'Category Management';
            const isOpen = isCategory ? isCategoryManagementOpen : isDocumentManagementOpen;
            const itemActive = isCategory ? isCategoryPathActive : isDocumentPathActive;

            return (
              <div key={index} className="space-y-1">
                <button
                  onClick={() => {
                    // Accordion: open this, close the other
                    if (isCategory) {
                      setIsCategoryManagementOpen((prev) => !prev);
                      setIsDocumentManagementOpen(false);
                    } else {
                      setIsDocumentManagementOpen((prev) => !prev);
                      setIsCategoryManagementOpen(false);
                    }
                  }}
                  className={`
                    ...
                    ${itemActive ? 'bg-opacity-10 shadow-md text-gray-900' : 'text-gray-600 hover:bg-gray-50 hover:translate-x-1'}
                  `}
                  style={itemActive ? { backgroundColor: brandLightPurple, color: brandPurple } : undefined}
                >
                  <div className="flex items-center space-x-3">
                    <dd.Icon className={itemActive ? 'text-brand-purple' : 'text-gray-400'} style={itemActive ? { color: brandPurple } : undefined} />
                    <span className="flex-1">{dd.label}</span>
                  </div>
            ...
            return (
              <Link
                key={(linkItem.href as string) || index}
                href={linkItem.href}
                className={`
                  relative flex items-center space-x-3 px-4 py-3 rounded-xl font-semibold 
                  transition-all duration-200 ease-in-out transform 
                  ${active ? 'bg-opacity-10 shadow-md' : 'text-gray-600 hover:bg-gray-50 hover:translate-x-1'}
                `}
                style={active ? activeStyle : undefined}
              >
                <linkItem.Icon className={active ? 'text-brand-purple' : 'text-gray-400'} style={active ? { color: brandPurple } : undefined} />
                ...
                {linkItem.badge && (
                  <span
                    className={`inline-flex items-center justify-center px-2 py-1 text-xs font-bold rounded-full transition-colors duration-200 ${
                      active ? 'text-white' : 'bg-brand-pink text-white'
                    }`}
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="flex-1 min-w-0 text-left">
              <p className="text-sm font-bold text-gray-800 truncate">{user?.email || 'Manager User'}</p>
              <p className="text-xs text-gray-500 capitalize">{user?.role?.replace('_', ' ') || 'Manager'}</p>
            </div>
            <ChevronDown className={`text-gray-500 transition-transform duration-300 group-hover:text-gray-800 ${isProfileOpen ? 'rotate-180' : ''}`} />
          </button>
          ...
          {isProfileOpen && (
            <div className="absolute left-0 right-0 z-20 py-2 mb-2 origin-bottom bg-white border border-gray-100 shadow-2xl bottom-full rounded-xl">
              <Link href="/manager/profile" className="flex items-center px-4 py-2 space-x-3 text-sm text-gray-700 transition-colors hover:bg-gray-50">
                <UserCircle className="text-gray-500" />
                <span>Profile Settings</span>
              </Link>

              <Link href="/manager/settings" className="flex items-center px-4 py-2 space-x-3 text-sm text-gray-700 transition-colors hover:bg-gray-50">
              ...
              <button onClick={logout} className="flex items-center w-full px-4 py-2 space-x-3 text-sm text-red-600 transition-colors group hover:bg-red-50">
